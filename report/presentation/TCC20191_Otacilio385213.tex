...
\begin{itemize} \justifying
	\item We are discussing the \textbf{optimal control} of \textbf{dynamical systems}. \vskip0.2cm
	...
	\item Optimal control theory has been revisited from several innovative fields in the last years\footfullcite{Tang:2016}\footfullcite{Eren:2017}. Furthermore, reactor systems are subject of active research, with several open challenges. \footfullcite{Mulas:2015}
\end{itemize}
...
\begin{equation}
\begin{cases}
	{\color{glgBlue!90} \dot{\bm{x}}(t)} = {\color{glgRed!80} \bm{f}(}{\color{glgBlue!90} \bm{x}(t)}, {\color{glgBlue!90} \bm{u}(t)} {\color{glgRed!80} )} \\
    {\color{glgBlue!90} \bm{y}(t)} = {\color{glgRed!80} \bm{g}(}{\color{glgBlue!90} \bm{x}(t)}, {\color{glgBlue!90} \bm{u}(t)} {\color{glgRed!80} )}
\end{cases}
...
\begin{itemize}
	\item[$\circ$] ${\color{glgBlue!90} \bm{x}(t)} \in \mathbb{R}^n$ is the \textit{state-vector}.
	
	\item[$\circ$] ${\color{glgBlue!90} \bm{u}(t)} \in \mathbb{R}^r$ is the \textit{input-vector}.
	
	\item[$\circ$] ${\color{glgBlue!90} \bm{y}(t)} \in \mathbb{R}^p$ is the \textit{output-vector}.
\end{itemize}
\end{minipage}
\begin{minipage}{0.54\textwidth}
\begin{itemize}
	\item[$\circ$] ${\color{glgRed!80} \bm{f}(\cdot)} : \mathbb{R}^{n\times r} \rightarrow \mathbb{R}^{n}$ is a \textit{state-transition function}.
	
	\item[$\circ$] ${\color{glgRed!80} \bm{g}(\cdot)} : \mathbb{R}^{n\times r} \rightarrow \mathbb{R}^{p}$ is a \textit{output observation function}.
\end{itemize}
\end{minipage}
\end{minipage}
...
\begin{figure}[ht] \centering
    ...
    \begin{tikzpicture}[auto, node distance=2cm,>=latex', scale=0.2]
        ...
        \draw [draw,->] (input) -- node[pos=0.1] {$\bm{u}$}  (system);
        \draw [->] (system) -- node[pos=0.9]{$\bm{y}$} (output);
...
\begin{equation}
\begin{cases}
	{\color{glgBlue!90} \dot{\bm{x}}(t)} = {\color{glgRed!80} \bm{A}}{\color{glgBlue!90} \bm{x}(t)} + {\color{glgRed!80} \bm{B}}{\color{glgBlue!90} \bm{u}(t)} \\
    {\color{glgBlue!90} \bm{y}(t)} = {\color{glgRed!80} \bm{C}}{\color{glgBlue!90} \bm{x}(t)} + {\color{glgRed!80} \bm{D}}{\color{glgBlue!90} \bm{u}(t)}
\end{cases}
\end{equation} \vskip0.2cm
...
\begin{itemize} \justifying
	\item[$\circ$] ${\color{glgRed!80} \bm{A}} \in \mathbb{R}^{n\times n}$ is the \textit{system matrix}.
	
	\item[$\circ$] ${\color{glgRed!80} \bm{B}} \in \mathbb{R}^{n\times r}$ is the \textit{input matrix}.
	
	\item[$\circ$] ${\color{glgRed!80} \bm{C}} \in \mathbb{R}^{p\times n}$ is the \textit{output matrix}.
	
	\item[$\circ$] ${\color{glgRed!80} \bm{D}} \in \mathbb{R}^{p\times r}$ is the \textit{feedthrough matrix}.
\end{itemize}
\end{minipage}
\end{minipage}
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[auto, node distance=2cm,>=latex', scale=0.2]
        ...
        % Once the nodes are placed, connecting them is easy.       
        \draw [draw,->] (input) -- node[pos=0.1] {$\bm{u}$} (inputMatrix);
        ...
        \draw [->] (outputMatrix) -- node[pos=0.9]{$\bm{y}$} (output);
...
\begin{itemize} \justifying
	...
	\begin{equation}
	\begin{cases}
        {\color{glgBlue!90} \bm{x}(t)} = e^{{\color{glgRed!80} \bm{A}}(t - t_0)} {\color{glgBlue!90} \bm{x}(t)} + \displaystyle\int_{t_0}^{t} e^{{\color{glgRed!80} \bm{A}}(t - \tau)} {\color{glgRed!80} \bm{B}} {\color{glgBlue!90} \bm{u}(\tau)} d \tau \hfill \\
        {\color{glgBlue!90} \bm{y}(t)} = {\color{glgRed!80} \bm{C}} e^{{\color{glgRed!80} \bm{A}}(t - t_0)} {\color{glgBlue!90} \bm{x}(t)} + {\color{glgRed!80} \bm{C}} \displaystyle\int_{t_0}^{t} e^{{\color{glgRed!80} \bm{A}}(t - \tau)} {\color{glgRed!80} \bm{B}} {\color{glgBlue!90} \bm{u}(\tau)} d \tau + {\color{glgRed!80} \bm{D}} {\color{glgBlue!90} \bm{u}(t)}
    \end{cases}
	\end{equation} 
	
	\item<2|only@2> The \textit{stability} of the model is directly related to the eigenvalues ($\bm{\lambda}$) of ${\color{glgRed!80} \bm{A}}$.
	\begin{equation}
	\begin{matrix}
		{\color{glgBlue!90} \bm{x}(t)} < \infty,\ {\color{glgBlue!90} t} \to \infty & \text{if } \text{Re}[\lambda_i] \leq 0,\ \forall i \in [1, n] 
	\end{matrix}
	\end{equation}
	
	\item<3|only@3> The \textit{controllability} property is directly related to the \textit{Controllability Matrix}:
	\begin{equation}
		\bm{\mathcal{C}} = \begin{bmatrix} {\color{glgRed!80} \bm{B}} & {\color{glgRed!80} \bm{A}} {\color{glgRed!80} \bm{B}} & {\color{glgRed!80} \bm{A}}^2 {\color{glgRed!80} \bm{B}} & \cdots & {\color{glgRed!80} \bm{A}}^{n-1} {\color{glgRed!80} \bm{B}} \end{bmatrix}
	\end{equation}
	
	\item<4|only@4> The \textit{observability} property is directly related to the \textit{Observability Matrix}:
	\begin{equation}
		\bm{\mathcal{O}} = \begin{bmatrix} {\color{glgRed!80} \bm{C}} & {\color{glgRed!80} \bm{C}} {\color{glgRed!80} \bm{A}} & {\color{glgRed!80} \bm{C}} {\color{glgRed!80} \bm{A}}^2  & \cdots & {\color{glgRed!80} \bm{C}} {\color{glgRed!80} \bm{A}}^{n-1} \end{bmatrix}^T
	\end{equation}
\end{itemize}
...
\begin{itemize} \justifying
	\item We choose $\bm{x} = [\rho_A, \rho_B, T, T_C]^T$, $\bm{u} = [q, Q]^T$ and $\bm{y} = [\rho_B, T]^T$. \vskip0.2cm
	
	\item We consider the steady-state point $\bm{x}_o = [1.23, 0.90, 134.14, 128.95]^T$ and $\bm{u}_o = [18.83, -4495.7]^T$.
\end{itemize}
...
\begin{itemize}
	...
	\begin{equation} 
		\texttt{rank}\left( \bm{\mathcal{C}} \right) = \texttt{rank}\left( \begin{bmatrix} {\color{glgRed!80} \bm{B}} & {\color{glgRed!80} \bm{A}} {\color{glgRed!80} \bm{B}} & {\color{glgRed!80} \bm{A}}^2 {\color{glgRed!80} \bm{B}} & {\color{glgRed!80} \bm{A}}^3 {\color{glgRed!80} \bm{B}} \end{bmatrix} \right) = 4
	\end{equation}
	
	\item<3-> The Observability Matrix has full-column rank. The system is \textbf{observable}.
	\begin{equation} 
		\texttt{rank}\left( \bm{\mathcal{O}} \right) = \texttt{rank}\left( \begin{bmatrix} {\color{glgRed!80} \bm{C}} & {\color{glgRed!80} \bm{C}} {\color{glgRed!80} \bm{A}} & {\color{glgRed!80} \bm{C}} {\color{glgRed!80} \bm{A}}^2 & {\color{glgRed!80} \bm{C}} {\color{glgRed!80} \bm{A}}^3 \end{bmatrix} \right) = 4
	\end{equation}
\end{itemize}
