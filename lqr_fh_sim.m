function [ y_out, x_out, x_hat, u_out, t_out ] = lqr_sim( A, B, C, D, Q, R, N, X0, X0_hat, t )
%LQR_FH_SIM Simulates the system with a Discrete-Time Finite-Horizon LQR Controller 
%   Detailed explanation goes here

    t_out = t'; x_out = zeros(size(A,2), numel(t)); x_hat = zeros(size(A,2), numel(t)); 
    u_out = zeros(size(B,2), numel(t)); X0 = X0';
    ...
    x_out(:, 1) = X0; x_hat(:, 1) = X0_hat;
    for i = 1:1:numel(t_out)-1
        if(i < N)
        u_out(:,i) = - pinv(R + B' * P(:,:,i+1) * B) * B' * P(:,:,i+1) * A * x_hat(:,i);
        end
        
        x_out(:, i+1) =  A * x_out(:,i) + B * u_out(:, i);
        x_hat(:, i+1) = (A - L * C)  * x_hat(:, i) + B * u_out(:, i) + L * C * x_out(:, i);
    end  

    y_out = C * x_out + D * u_out;
    
    y_out = y_out'; x_out = x_out'; x_hat = x_hat'; u_out = u_out'; 
end
