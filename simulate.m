...
%SIMULATE Simulates a non-linear model using the Euler method.
    ...
    switch length(varargin)
        ...
        % == SWITCHING MODE LINEAR QUADRATIC CONTROLLER ==,
        ...
        elseif(strcmp(type, 'switch-lqgi'))
            % Estimate the covariances for the process and measurement noises
            Q_k = cov(w);
            R_k = cov((C * z')');
            P_k = zeros(2);
            ...
            if(N ~= 'inf') 
                ...
                for i = 1:numel(t)-1
                    ...
                    % Calculates the input signal (if still on control horizon)
                    if(2 <= i && i < N)
                        u(:,i) = u(:,i-1) - K(i,:) * [x_hat(:, i-1) - x_hat(:, i); (r(:,i) - C*xe) - C_d * x_hat(:,i)];
                    end
